...
(function () {
    ...
    function extractNodeLabel(nodeId) {
        ...
        if (parts.length >= 3) {
            ...
            if (type === 'input') {
                if (network === 'time') {
                    const timeInputNames = { '-1': 'rawTime', '-2': 'mSpeed', '-3': 'mDist', '-4': 'inact', '-5': 'bias' };
                    return timeInputNames[id] || 'in' + id;
                }
                const visualInputNames = { '-1': 'x', '-2': 'y', '-3': 'dist', '-4': 'time', '-5': 'mSpeed', '-6': 'mDist', '-7': 'inact', '-8': 'bias' };
    ...
    function visualizeNetworkInline(individualId, data, container) {
        if (typeof vis === 'undefined') {
            if (typeof showToast === 'function') showToast('Visualization error', 'vis.js not loaded', 'error');
            return;
        }
        try {
            ...
            data.connections.forEach(function (conn) {
                ...
                let color;
                if (network === 'time') {
                    color = isPositive ? 'rgba(0, 150, 200, 0.3)' : 'rgba(200, 150, 0, 0.3)';
                } else {
                    color = isPositive ? 'rgba(0, 200, 100, 0.5)' : 'rgba(255, 100, 100, 0.5)';
                }
                edges.add({
                    from: conn.source,
                    to: conn.target,
            ...
            const options = {
                ...
                edges: { font: { size: 8, align: 'middle' }, smooth: { enabled: true, type: 'cubicBezier', forceDirection: 'vertical' } }
            };

            const network = new vis.Network(vizContainer, { nodes: nodes, edges: edges }, options);
            ...
            if (typeof showToast === 'function') showToast('Visualization error', err.message || 'Failed to visualize network', 'error');
        }
    }
    ...
    function setupWeightSliders(individualId, data) {
        ...
        const visualConns = data.connections.filter(function (c) { return c.network === 'visual'; });
        const timeConns = data.connections.filter(function (c) { return c.network === 'time'; });
        visualConns.forEach(function (conn) { createWeightSlider(individualId, conn, 'visual', container); });
        timeConns.forEach(function (conn) { createWeightSlider(individualId, conn, 'time', container); });
    }
    ...
    function createWeightSlider(individualId, connection, networkType, container) {
        ...
        sliderDiv.innerHTML =
            ...
            '<input type="range" class="weight-slider-input" min="' + minWeight + '" max="' + maxWeight + '" step="0.05" value="' + currentWeight + '" ' +
            'data-individual="' + individualId + '" data-network="' + networkType + '" data-source="' + connection.source + '" data-target="' + connection.target + '" />' +
            '<span class="weight-value">' + currentWeight.toFixed(2) + '</span></div>';
        ...
        slider.addEventListener('input', function (e) {
            ...
            _getGenomeForPattern(individualId).then(function (genome) {
                if (!genome) {
                    if (typeof showToast === 'function') showToast('Weight update failed', 'Could not find genome data', 'error');
                    return;
                }
                fetch(_apiUrl + '/adjust-weight', {
                    ...
                    .then(function (data) {
                        if (data.status === 'success') {
                            if (typeof _updatePatternShader === 'function') _updatePatternShader(individualId, data.shader);
                            if (typeof _onGenomeUpdated === 'function' && data.genome && typeof _getCurrentPopulation === 'function') {
                                const pop = _getCurrentPopulation();
                                if (pop && pop.length) {
                                    const idx = pop.findIndex(function (p) { return p.id === individualId; });
                                    if (idx >= 0) _onGenomeUpdated(individualId, idx, data.genome);
                                }
                            }
                            updateNetworkEdgeWeight(individualId, connection.source, connection.target, newWeight, networkType);
                        } else {
                            if (typeof showToast === 'function') showToast('Weight update failed', data.error || 'Server error', 'error');
                        }
                    })
                    .catch(function (err) {
                        if (typeof showToast === 'function') showToast('Weight update failed', err.message || 'Network error', 'error');
                    });
            });
        });
    ...
    function updateNetworkEdgeWeight(individualId, sourceId, targetId, newWeight, networkType) {
        ...
        for (let i = 0; i < allEdges.length; i++) {
            ...
            if (edge.from === sourceId && edge.to === targetId) {
                ...
                edges.update({ id: edge.id, width: width, color: color, title: networkType.toUpperCase() + ' Weight: ' + newWeight.toFixed(3) });
                break;
            }
        }
    }
    ...
    function scrollToWeightSlider(sourceNodeId, targetNodeId) {
        ...
        sliderItems.forEach(function (item) { item.classList.remove('highlighted'); });
        for (let i = 0; i < sliderItems.length; i++) {
            const item = sliderItems[i];
            if (item.getAttribute('data-source') === sourceNodeId && item.getAttribute('data-target') === targetNodeId) {
                item.classList.add('highlighted');
                item.scrollIntoView({ behavior: 'smooth', block: 'center' });
                setTimeout(function () { item.classList.remove('highlighted'); }, 2000);
                break;
            }
        }
    }

    async function toggle(individualId, card) {
        ...
        if (typeof _getGenomeForPattern !== 'function') {
            if (typeof showToast === 'function') showToast('Network error', 'Not initialized', 'error');
            return;
        }
        const genome = await _getGenomeForPattern(individualId);
        if (!genome) {
            if (typeof showToast === 'function') showToast('Network error', 'Could not find genome data for this pattern', 'error');
            return;
        }
        try {
            ...
            if (data.error) {
                if (typeof showToast === 'function') showToast('Network error', data.error, 'error');
                return;
            }
            const infoPanel = sidebar.querySelector('.network-info-panel');
            if (infoPanel) {
                const visualNodes = data.nodes.filter(function (n) { return n.network === 'visual' && n.type === 'hidden'; }).length;
                const timeNodes = data.nodes.filter(function (n) { return n.network === 'time' && n.type === 'hidden'; }).length;
                const visualConns = data.connections.filter(function (c) { return c.network === 'visual'; }).length;
                const timeConns = data.connections.filter(function (c) { return c.network === 'time'; }).length;
                const nodesEl = infoPanel.querySelector('[data-nodes]');
                const connsEl = infoPanel.querySelector('[data-connections]');
                if (nodesEl) nodesEl.textContent = 'V:' + visualNodes + ' T:' + timeNodes;
                if (connsEl) connsEl.textContent = 'V:' + visualConns + ' T:' + timeConns;
            }
            visualizeNetworkInline(individualId, data, sidebar);
            sidebar.classList.add('open');
        } catch (err) {
            if (typeof showToast === 'function') showToast('Network error', err.message || 'Failed to load network', 'error');
        }
    }
    ...
    function exportNetwork() {
        ...
        const exportData = { id: currentNetworkId, nodes: data.nodes.get(), edges: data.edges.get() };
    ...
    window.NetworkVisualizer = {
        ...
        init: init,
        get currentId() { return currentNetworkId; }
    };
})();
