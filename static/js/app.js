...
(function () {
    ...
    var API_URL = (window.location.origin && window.location.protocol.indexOf('http') === 0)
    ...
    function renderPattern(patternData, time, mouseSpd, mouseDist, inact, mousePos) {
        if (window.PatternRenderer && window.ZoomSignals) {
            window.PatternRenderer.renderPattern(patternData, time, mouseSpd, mouseDist, inact, window.ZoomSignals.signalState, mousePos);
        }
    }

    // Compute mouse position within a canvas (normalized 0-1)
    function getMousePositionOnCanvas(canvas) {
        var rect = canvas.getBoundingClientRect();
        var x = Math.max(0, Math.min(1, (mouseX - rect.left) / rect.width));
        // Flip Y: browser has Y=0 at top, WebGL has Y=0 at bottom
        var y = 1.0 - Math.max(0, Math.min(1, (mouseY - rect.top) / rect.height));
        return { x: x, y: y };
    }
    ...
    function showGridError(message, showRetry) {
        ...
        if (showRetry) {
            document.getElementById('grid-retry-btn').onclick = function () { window.PopulationUI.startNewRandomPopulation(); };
        }
    }
        ...
        population.forEach(function (pattern) {
            var result = window.PatternRenderer.createPatternCard({
                ...
                onMouseEnter: function (id) {
                    if (typeof window.EyecatcherDebug !== 'undefined') window.EyecatcherDebug.setHoveredPatternId(id);
                },
                onMouseLeave: function (id) {
                    if (typeof window.EyecatcherDebug !== 'undefined' && window.EyecatcherDebug.getHoveredPatternId() === id) {
    ...
    function addToGrid(genomes) {
        ...
        return window.ApiClient.compile(payload, getColorMode())
            ...
            .catch(function (e) {
                console.error(e);
                if (window.Toast) window.Toast.show('Add failed', e.message || 'Failed to compile', 'error');
            })
            .then(function () {
                document.getElementById('loading').style.display = 'none';
            });
    ...
    function breedGeneration() {
        ...
        if (!currentGenomes) {
            if (window.Toast) window.Toast.error('No population loaded. Start with New random population or Load population.');
            document.getElementById('loading').style.display = 'none';
        ...
        var parents = currentPopulation
            ...
            .filter(Boolean)
            .filter(function (p) { return p.clicks > 0; });

        if (!parents.length) {
            if (window.Toast) window.Toast.error('Select at least one pattern (click on it) before breeding.');
            document.getElementById('loading').style.display = 'none';
            updateStats();
            setBreedButtonDisabled(false);
            return;
        ...
        window.ApiClient.breed(parents, populationSize, {
            ...
            .catch(function (e) {
                ...
                if (window.Toast) window.Toast.error('Breed failed: ' + (e.message || String(e)));
                document.getElementById('loading').style.display = 'none';
                updateStats();
                setBreedButtonDisabled(false);
            });
    ...
    function savePattern(id, buttonEl) {
        if (!currentGenomes || !currentGenomes.length) {
            if (window.Toast) window.Toast.show('Cannot save', 'No pattern data. Start with New random population or Load population.', 'error');
            return;
        }
        var idx = currentPopulation.findIndex(function (p) { return p.id === id; });
        var genome = idx >= 0 && currentGenomes[idx] ? currentGenomes[idx] : null;
        if (!genome) {
            if (window.Toast) window.Toast.show('Cannot save', 'Could not get pattern data.', 'error');
            return;
        }
        ...
        window.ApiClient.save(id, genome)
            .then(function (data) {
                if (Array.isArray(data.downloads) && data.downloads.length) {
                    ...
                    if (window.Toast) window.Toast.show('Pattern saved!', 'Zip downloaded to your computer.', 'success', { duration: 5000 });
                } else {
                    if (window.Toast) window.Toast.show('Pattern saved!', 'No download in response.', 'success');
                }
            })
            .catch(function (error) {
                console.error('Error saving:', error);
                if (window.Toast) window.Toast.show('Save failed', error.message || 'Network error', 'error');
            })
            .then(function () {
                if (buttonEl) {
    ...
    function getGenomeForPattern(patternId) {
        ...
        var idx = currentPopulation.findIndex(function (p) { return p.id === patternId; });
        var genome = idx >= 0 && currentGenomes[idx] ? currentGenomes[idx] : null;
    ...
    function updateStats() {
        var totalClicks = 0;
        patterns.forEach(function (p) { totalClicks += p.clicks || 0; });
        var hasFitness = false;
        patterns.forEach(function (p) { if (p.clicks > 0) hasFitness = true; });
    ...
    function updatePatternShader(individualId, newShader) {
        ...
        if (pattern && window.PatternRenderer) {
            var newPatternData = window.PatternRenderer.setupPattern(pattern.canvas, newShader);
            if (newPatternData) {
    ...
    window.NetworkVisualizer.init({
        ...
        getCurrentPopulation: function () { return currentPopulation; },
    ...
    if (breedBtnEl) {
        ...
        breedBtnEl.addEventListener('keydown', function (e) { onRoleButtonKeydown(e, breedGeneration); });
    }

    var loadModalClose = document.getElementById('load-modal-close');
    if (loadModalClose) loadModalClose.addEventListener('click', function () {
        document.getElementById('load-list-modal').classList.remove('show');
    });
    document.getElementById('community-submit-do').addEventListener('click', window.CommunityUI.submitCommunityForm);
    document.getElementById('community-submit-cancel').addEventListener('click', window.CommunityUI.closeSubmitCommunityModal);
    document.getElementById('community-list-close').addEventListener('click', function () {
        document.getElementById('community-list-modal').classList.remove('show');
    });
    document.getElementById('community-load-selected-btn').addEventListener('click', window.CommunityUI.onCommunityLoadSelected);
    document.getElementById('community-load-12-btn').addEventListener('click', window.CommunityUI.onCommunityLoad12);
    document.getElementById('community-select-all-btn').addEventListener('click', window.CommunityUI.onCommunitySelectAll);
    document.getElementById('community-deselect-all-btn').addEventListener('click', window.CommunityUI.onCommunityDeselectAll);
    var newFromCommunityBtn = document.getElementById('new-from-community-btn');
    if (newFromCommunityBtn) {
        newFromCommunityBtn.addEventListener('click', window.CommunityUI.onNewFromCommunityClick);
        newFromCommunityBtn.addEventListener('keydown', function (e) { onRoleButtonKeydown(e, window.CommunityUI.onNewFromCommunityClick); });
    }
    document.getElementById('admin-key-submit').addEventListener('click', window.CommunityUI.submitAdminKey);
    document.getElementById('admin-modal-cancel').addEventListener('click', window.CommunityUI.closeAdminModal);
    document.getElementById('admin-list-close').addEventListener('click', window.CommunityUI.closeAdminModal);
    ...
    if (saveCurrentBtn) saveCurrentBtn.addEventListener('click', window.PopulationUI.onSaveCurrentClick);
    var importBtn = document.getElementById('import-btn');
    var importFile = document.getElementById('import-file');
    if (importBtn && importFile) importBtn.addEventListener('click', window.PopulationUI.onImportClick);
    if (importFile) {
    ...
    if (typeof window.EyecatcherDebug !== 'undefined') {
        window.EyecatcherDebug.init({
            ...
            getSignalState: function () { return window.ZoomSignals.signalState; },
            getGenomeForPattern: getGenomeForPattern
        });
    }
