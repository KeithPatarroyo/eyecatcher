...
(function () {
    ...
    function getMousePositionOnCanvas(canvas) {
        ...
        const y = 1.0 - Math.max(0, Math.min(1, (mouseY - rect.top) / rect.height));
        return { x: x, y: y };
    }
        ...
        if (animating) {
            ...
            if (patterns && _renderPattern) {
                patterns.forEach(function (patternData) {
                    ...
                    const mousePos = getMousePositionOnCanvas(patternData.canvas);
                    _renderPattern(patternData, normalizedTime, mouseSpeed, mouseDist, activity, mousePos);
                });
            }
