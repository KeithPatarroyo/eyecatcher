...
def test_compile_dual_empty_connections(substrate, random_dual_genome):
...
def test_compile_dual_single_hidden_node(substrate):
    """Compiler with exactly one hidden node in visual CPPN (deterministic)."""
    dual = minimal_dual_genome_one_hidden_visual(substrate)
    hidden_visual = [
        n for n in dual.visual.nodes if n >= substrate.config.genome_config.num_outputs
    ]
    assert (
        len(hidden_visual) == 1
