...
| test_api.py | Flask API: /api/random, /api/compile, /api/evolve (with/without genealogy), /api/save, save download structure, /api/adjust-weight. Uses test client and substrate/minimal_dual fixtures. |
| test_genealogy_routes.py | Genealogy API: save-population, load-population, tree, branches, export (full and by branch), reset, stats; uses genealogy_db and substrate. |
| test_community_routes.py | Community API: submit, list, admin approve/reject; uses community_db and substrate. |
| test_visualization.py | Genome visualizer PDF and render_image (single-CPPN); uses tmp_path, substrate. |

## Fixtures (conftest.py)

- **client** – Flask test client (app.test_client() with TESTING=True).
- **substrate** – DualCPPNSubstrate instance (for mutation, crossover, evaluate tests).
