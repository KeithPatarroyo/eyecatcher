...
def get_network_data(individual_id):
    """Get both CPPN networks (visual and time signal) for visualization."""
    for individual in current_population:
        if individual['id'] == individual_id:
            all_nodes = []
            all_connections = []
            ...
            if hasattr(individual['genome'], 'visual'):
                visual_genome = individual['genome'].visual
                visual_nodes, visual_conns = extract_network_data(visual_genome, 'visual', engine.config)
                all_nodes.extend(visual_nodes)
                all_connections.extend(visual_conns)
            
            # Extract time signal network
            if hasattr(individual['genome'], 'time_signal'):
                time_genome = individual['genome'].time_signal
                time_nodes, time_conns = extract_network_data(time_genome, 'time', engine.time_config)
